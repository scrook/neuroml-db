TITLE Mod file for component: Component(id=Channelpedia_KSlow_30 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Channelpedia_KSlow_30 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Channelpedia_KSlow_30 type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = ((1.25+175.03*exp(- m_timeCourse_V  * -0.026)))* m_timeCourse_TIME_SCALE ? evaluable
    m_steadyState_V = v /  m_steadyState_VOLT_SCALE ? evaluable
    m_steadyState_x = (1/(1 + exp(-( m_steadyState_V +14)/14.6))) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHtauInf), from steadyState; Component(id=null type=Channelpedia_KSlow_30_m_inf)
    m_inf = m_steadyState_x ? path based, prefix = m_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=Channelpedia_KSlow_30_m_tau)
    ...
    h_timeCourse_t = (360+(1010+24*( h_timeCourse_V +55))*exp(-(( h_timeCourse_V +75)/48)^2))* h_timeCourse_TIME_SCALE ? evaluable
    h_steadyState_V = v /  h_steadyState_VOLT_SCALE ? evaluable
    h_steadyState_x = 1/(1 + exp(-( h_steadyState_V +54)/-11)) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=h type=gateHHtauInf), from steadyState; Component(id=null type=Channelpedia_KSlow_30_h_inf)
    h_inf = h_steadyState_x ? path based, prefix = h_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=Channelpedia_KSlow_30_h_tau)
