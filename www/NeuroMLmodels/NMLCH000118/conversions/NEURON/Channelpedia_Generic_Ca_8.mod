TITLE Mod file for component: Component(id=Channelpedia_Generic_Ca_8 type=ionChannelHH)
...
NEURON {
    SUFFIX Channelpedia_Generic_Ca_8
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Channelpedia_Generic_Ca_8 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Channelpedia_Generic_Ca_8 type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    m_forwardRate_r = (0.055*(-27- m_forwardRate_V ))/(exp((-27- m_forwardRate_V )/3.8) - 1) /  m_forwardRate_TIME_SCALE ? evaluable
    m_reverseRate_V = v /  m_reverseRate_VOLT_SCALE ? evaluable
    m_reverseRate_r = (0.94*exp((-75- m_reverseRate_V )/17))  /  m_reverseRate_TIME_SCALE ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=m type=gateHHrates), from q10Settings; null
    ? Path not present in component, using factor: 1
    
    m_rateScale = 1 
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=m type=gateHHrates), from forwardRate; Component(id=null type=Channelpedia_Generic_Ca_8_m_alpha)
    m_alpha = m_forwardRate_r ? path based, prefix = m_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=m type=gateHHrates), from reverseRate; Component(id=null type=Channelpedia_Generic_Ca_8_m_beta)
    ...
    h_forwardRate_r = (0.000457*exp((-13- h_forwardRate_V )/50)) /  h_forwardRate_TIME_SCALE ? evaluable
    h_reverseRate_V = v /  h_reverseRate_VOLT_SCALE ? evaluable
    h_reverseRate_r = (0.0065/(exp((- h_reverseRate_V -15)/28)+1))  /  h_reverseRate_TIME_SCALE ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=h type=gateHHrates), from q10Settings; null
    ? Path not present in component, using factor: 1
    
    h_rateScale = 1 
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHrates), from forwardRate; Component(id=null type=Channelpedia_Generic_Ca_8_h_alpha)
    h_alpha = h_forwardRate_r ? path based, prefix = h_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHrates), from reverseRate; Component(id=null type=Channelpedia_Generic_Ca_8_h_beta)
