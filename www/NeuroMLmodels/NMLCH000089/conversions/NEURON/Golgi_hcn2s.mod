TITLE Mod file for component: Component(id=Golgi_hcn2s type=ionChannelHH)
...
ASSIGNED {
    ...
    s_steadyState_x                        : conditional derived var...
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Golgi_hcn2s type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Golgi_hcn2s type=ionChannelHH), from gates; Component(id=s type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    s_timeCourse_t = ((exp (2.3026* (0.0152 *  s_timeCourse_V  + 5.2944)))) *  s_timeCourse_TIME_SCALE ? evaluable
    s_steadyState_V = v /  s_steadyState_VOLT_SCALE ? evaluable
    if (s_steadyState_V   < ( -64.70 ))  { 
        s_steadyState_x = (1 + 0.0227  *  s_steadyState_V   + 1.4694) *  1 / (1 + (exp (( s_steadyState_V  + 81.95) * 0.1661 ))) ? evaluable cdv
    } else  { 
        s_steadyState_x = 1 / (1 + (exp (( s_steadyState_V  + 81.95) * 0.1661 ))) ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=s type=gateHHtauInf), from steadyState; Component(id=null type=Golgi_hcn2s_s_inf_inf)
    s_inf = s_steadyState_x ? path based, prefix = s_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=s type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_hcn2s_s_tau_tau)
