TITLE Mod file for component: Component(id=Channelpedia_Na_S_36 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Channelpedia_Na_S_36 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Channelpedia_Na_S_36 type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    m_forwardRate_r = (0.182 * (( m_forwardRate_V -10)+35))/(1-(exp(-(( m_forwardRate_V -10)+35)/9))) /  m_forwardRate_TIME_SCALE ? evaluable
    m_reverseRate_V = v /  m_reverseRate_VOLT_SCALE ? evaluable
    m_reverseRate_r = (0.124 * (-( m_reverseRate_V -10) -35))/(1-(exp(-(-( m_reverseRate_V -10) -35)/9)))  /  m_reverseRate_TIME_SCALE ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=m type=gateHHrates), from q10Settings; null
    ? Path not present in component, using factor: 1
    
    m_rateScale = 1 
    
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=m type=gateHHrates), from forwardRate; Component(id=null type=Channelpedia_Na_S_36_m_alpha)
    m_alpha = m_forwardRate_r ? path based, prefix = m_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=m type=gateHHrates), from reverseRate; Component(id=null type=Channelpedia_Na_S_36_m_beta)
    ...
    h_timeCourse_t = (1/((0.024 * (( h_timeCourse_V -10)+50))/(1-(exp(-(( h_timeCourse_V -10)+50)/5))) +(0.0091 * (-( h_timeCourse_V -10) - 75.000123))/(1-(exp(-(-( h_timeCourse_V -10) - 75.000123)/5))))) *  h_timeCourse_TIME_SCALE ? evaluable
    h_steadyState_V = v /  h_steadyState_VOLT_SCALE ? evaluable
    h_steadyState_x = 1.0/(1+exp(( h_steadyState_V +65-10)/6.2)) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=h type=gateHHtauInf), from steadyState; Component(id=null type=Channelpedia_Na_S_36_h_inf)
    h_inf = h_steadyState_x ? path based, prefix = h_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=Channelpedia_Na_S_36_h_tau)
