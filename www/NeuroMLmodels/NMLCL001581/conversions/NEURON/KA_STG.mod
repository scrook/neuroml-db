TITLE Mod file for component: Component(id=KA_STG type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -27.2 (mV)
    m_steadyState_scale = 8.7 (mV)
    h_instances = 1 
    h_timeCourse_TIME_SCALE = 1 (ms)
    h_timeCourse_VOLT_SCALE = 1 (mV)
    h_steadyState_rate = 1 
    h_steadyState_midpoint = -56.9 (mV)
    h_steadyState_scale = -4.9 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=KA_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=KA_STG type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (23.2 - 20.8/(1.0 + (exp (( m_timeCourse_V  + 32.9)/(-15.2))))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=KA_STG_m_tau_tau)
    ...
    h_timeCourse_t = (77.2 - 58.4/(1.0 + (exp (( h_timeCourse_V  + 38.9)/(-26.5))))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=KA_STG_h_tau_tau)
