TITLE Mod file for component: Component(id=KvAdistp type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=KvAdistp type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=KvAdistp type=ionChannelHH), from gates; Component(id=n type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    n_timeCourse_zeta = -1.8 - (1 / (1 + exp(( n_timeCourse_V +40)/5))) ? evaluable
    n_timeCourse_alpha = exp((1e-3* n_timeCourse_zeta *( n_timeCourse_V +1)* n_timeCourse_F ) / ( n_timeCourse_R *( n_timeCourse_T +0.01))) ? evaluable
    n_timeCourse_beta = exp((1e-3* n_timeCourse_zeta *0.39*( n_timeCourse_V +1)* n_timeCourse_F ) / ( n_timeCourse_R *( n_timeCourse_T +0.01))) ? evaluable
    n_timeCourse_tmp = n_timeCourse_beta  / (0.1*(1 +  n_timeCourse_alpha )) ? evaluable
    if (( n_timeCourse_tmp  /  n_rateScale ) < 0.2)  { 
    ...
    n_steadyState_zeta = -1.8 - (1 / (1 + exp(( n_steadyState_V +40)/5))) ? evaluable
    n_steadyState_alpha = exp((1e-3* n_steadyState_zeta *( n_steadyState_V +1)* n_steadyState_F ) / ( n_steadyState_R *( n_steadyState_T +0.01))) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=n type=gateHHtauInf), from steadyState; Component(id=null type=Bezaire_KvAdistp_ssn)
    n_inf = n_steadyState_x ? path based, prefix = n_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHtauInf), from timeCourse; Component(id=null type=Bezaire_KvAdistp_taun)
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=l type=gateHHtauInf), from steadyState; Component(id=null type=Bezaire_KvAdistp_ssl)
    l_inf = l_steadyState_x ? path based, prefix = l_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=l type=gateHHtauInf), from timeCourse; Component(id=null type=Bezaire_KvAdistp_taul)
