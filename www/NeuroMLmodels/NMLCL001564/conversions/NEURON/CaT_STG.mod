TITLE Mod file for component: Component(id=CaT_STG type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -27.1 (mV)
    m_steadyState_scale = 7.2 (mV)
    h_instances = 1 
    h_timeCourse_TIME_SCALE = 1 (ms)
    h_timeCourse_VOLT_SCALE = 1 (mV)
    h_steadyState_rate = 1 
    h_steadyState_midpoint = -32.1 (mV)
    h_steadyState_scale = -5.5 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=CaT_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=CaT_STG type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (43.4 - 42.6/(1.0 + (exp (( m_timeCourse_V  + 68.1)/(-20.5))))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=CaT_STG_m_tau_tau)
    ...
    h_timeCourse_t = (210 - 179.6/(1.0 + (exp (( h_timeCourse_V  + 55)/(-16.9))))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=CaT_STG_h_tau_tau)
