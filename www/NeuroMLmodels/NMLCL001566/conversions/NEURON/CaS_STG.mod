TITLE Mod file for component: Component(id=CaS_STG type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=CaS_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=CaS_STG type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (2.8+14/((exp (( m_timeCourse_V  + 27)/10)) + (exp (( m_timeCourse_V  + 70)/(-13))))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=CaS_STG_m_tau_tau)
    ...
    h_timeCourse_t = (120+300/( (exp (( h_timeCourse_V  + 55)/9)) + (exp (( h_timeCourse_V  + 65)/(-16))))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=CaS_STG_h_tau_tau)
