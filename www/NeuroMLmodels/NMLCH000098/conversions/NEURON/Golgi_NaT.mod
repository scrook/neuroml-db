TITLE Mod file for component: Component(id=Golgi_NaT type=ionChannelHH)
...
PARAMETER {
    ...
    m_instances = 3 
    m_forwardRate_rate = 3.0000002 (kHz)
    m_forwardRate_midpoint = -25 (mV)
    m_forwardRate_scale = 10 (mV)
    m_reverseRate_rate = 12.000001 (kHz)
    m_reverseRate_midpoint = -50 (mV)
    m_reverseRate_scale = -18.182 (mV)
    m_q10Settings_q10Factor = 3 
    m_q10Settings_experimentalTemp = 293.15 (K)
    m_q10Settings_TENDEGREES = 10 (K)
    h_instances = 1 
    h_forwardRate_rate = 0.21000001 (kHz)
    h_forwardRate_midpoint = -50 (mV)
    h_forwardRate_scale = -3.333 (mV)
    h_reverseRate_TIME_SCALE = 1 (ms)
    h_reverseRate_VOLT_SCALE = 1 (mV)
    h_q10Settings_q10Factor = 3 
    h_q10Settings_experimentalTemp = 293.15 (K)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Golgi_NaT type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Golgi_NaT type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    h_reverseRate_r = (3/(1+ (exp (( h_reverseRate_V +17)/-5)))) /  h_reverseRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHrates), from reverseRate; Component(id=null type=Golgi_NaT_h_beta_rate)
