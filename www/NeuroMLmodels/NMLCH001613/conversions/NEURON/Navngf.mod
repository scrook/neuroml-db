TITLE Mod file for component: Component(id=Navngf type=ionChannelHH)
...
PARAMETER {
    ...
    m_instances = 3 
    m_forwardRate_rate = 1.7066501 (kHz)
    m_forwardRate_midpoint = -24 (mV)
    m_forwardRate_scale = 5 (mV)
    m_reverseRate_rate = 1.4241501 (kHz)
    m_reverseRate_midpoint = 4 (mV)
    ...
    h_reverseRate_rate = 3.0931 (kHz)
    h_reverseRate_midpoint = -12.1463 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Navngf type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Navngf type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    h_forwardRate_r = ( 0.29648 / (exp(( h_forwardRate_V +64.4184)/20))) /  h_forwardRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHrates), from forwardRate; Component(id=null type=Bezaire_Navngf_alphah)
