TITLE Mod file for component: Component(id=Gran_NaF_98 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Gran_NaF_98 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Gran_NaF_98 type=ionChannelHH), from gates; Component(id=m type=gateHHratesTau)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHratesTau), from timeCourse; Component(id=null type=Gran_NaF_98_m_tau_tau)
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHratesTau), from timeCourse; Component(id=null type=Gran_NaF_98_h_tau_tau)
