TITLE Mod file for component: Component(id=Kdrp type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Kdrp type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Kdrp type=ionChannelHH), from gates; Component(id=n type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    n_timeCourse_alpha = exp((1e-3*(-3)*( n_timeCourse_V -13)* n_timeCourse_F ) / ( n_timeCourse_R *( n_timeCourse_T +0.01))) ? evaluable
    n_timeCourse_beta = exp((1e-3*(-3)*0.7*( n_timeCourse_V -13)* n_timeCourse_F ) / ( n_timeCourse_R *( n_timeCourse_T +0.01))) ? evaluable
    n_timeCourse_tmp = n_timeCourse_beta  / (0.02*(1 +  n_timeCourse_alpha )) ? evaluable
    if (( n_timeCourse_tmp  /  n_rateScale ) < 2)  { 
        n_timeCourse_t = 2 *  n_rateScale  *  n_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
    ...
    n_steadyState_alpha = exp((1e-3*(-3)*( n_steadyState_V -13)* n_steadyState_F ) / ( n_steadyState_R *( n_steadyState_T +0.01))) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=n type=gateHHtauInf), from steadyState; Component(id=null type=Bezaire_Kdrp_ss)
    n_inf = n_steadyState_x ? path based, prefix = n_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHtauInf), from timeCourse; Component(id=null type=Bezaire_Kdrp_tau)
