TITLE Mod file for component: Component(id=Gran_KDr_98 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Gran_KDr_98 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Gran_KDr_98 type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    m_forwardRate_r = (170 * ((exp (73 *( m_forwardRate_V  - (-0.038)))))) /  m_forwardRate_TIME_SCALE ? evaluable
    m_reverseRate_V = (v -  m_reverseRate_offset ) /  m_reverseRate_VOLT_SCALE ? evaluable
    m_reverseRate_r = (170 * ((exp ((-18) *( m_reverseRate_V  - (-0.038)))))) /  m_reverseRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=m type=gateHHrates), from forwardRate; Component(id=null type=Gran_KDr_98_m_alpha_rate)
    m_alpha = m_forwardRate_r ? path based, prefix = m_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=m type=gateHHrates), from reverseRate; Component(id=null type=Gran_KDr_98_m_beta_rate)
    ...
    h_forwardRate_V = (v -  h_forwardRate_offset ) /  h_forwardRate_VOLT_SCALE ? evaluable
    if (h_forwardRate_V   > ( -0.046 ))  { 
        h_forwardRate_r = 0.76  /  h_forwardRate_TIME_SCALE ? evaluable cdv
    } else  { 
        h_forwardRate_r = ( 0.7 + 0.065*(exp (-80*( h_forwardRate_V  - (-0.046))))) /  h_forwardRate_TIME_SCALE ? evaluable cdv
    }
    
    h_reverseRate_V = (v -  h_reverseRate_offset ) /  h_reverseRate_VOLT_SCALE ? evaluable
    h_reverseRate_r = (1.1/(1 + (exp (-80.7 * ( h_reverseRate_V  - (-0.044)))))) /  h_reverseRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHrates), from forwardRate; Component(id=null type=Gran_KDr_98_h_alpha_rate)
    h_alpha = h_forwardRate_r ? path based, prefix = h_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHrates), from reverseRate; Component(id=null type=Gran_KDr_98_h_beta_rate)
