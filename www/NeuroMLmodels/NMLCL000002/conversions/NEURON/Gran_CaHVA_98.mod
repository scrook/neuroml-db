TITLE Mod file for component: Component(id=Gran_CaHVA_98 type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Gran_CaHVA_98 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Gran_CaHVA_98 type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    if (h_forwardRate_V    < ( -0.060 ))  { 
        h_forwardRate_r = 5.0  /  h_forwardRate_TIME_SCALE ? evaluable cdv
    } else  { 
        h_forwardRate_r = ( 5 * (exp (-50 * ( h_forwardRate_V  - (-0.060))))) /  h_forwardRate_TIME_SCALE ? evaluable cdv
    }
    
    h_reverseRate_V = (v -  h_reverseRate_offset ) /  h_reverseRate_VOLT_SCALE ? evaluable
    if (h_reverseRate_V    < ( -0.060 ))  { 
        h_reverseRate_r = 0.0  /  h_reverseRate_TIME_SCALE ? evaluable cdv
    } else  { 
        h_reverseRate_r = ( 5 - (5 * (exp (-50 * ( h_reverseRate_V  - (-0.060)))))) /  h_reverseRate_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHrates), from forwardRate; Component(id=null type=Gran_CaHVA_98_h_alpha_rate)
    h_alpha = h_forwardRate_r ? path based, prefix = h_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=h type=gateHHrates), from reverseRate; Component(id=null type=Gran_CaHVA_98_h_beta_rate)
