TITLE Mod file for component: Component(id=Navp type=ionChannelHH)
...
NEURON {
    ...
    RANGE s_timeCourse_TEMP_SCALE           : parameter
    RANGE s_timeCourse_R                    : parameter
    RANGE s_timeCourse_F                    : parameter
    
    RANGE s_timeCourse_t                    : exposure
    RANGE s_steadyState_no_inact            : parameter
    RANGE s_steadyState_VOLT_SCALE          : parameter
    
    RANGE s_steadyState_x                   : exposure
    RANGE s_q10Settings_fixedQ10            : parameter
...
PARAMETER {
    ...
    s_timeCourse_TEMP_SCALE = 1 (K)
    s_timeCourse_R = 8.315 
    s_timeCourse_F = 96480 
    s_steadyState_no_inact = 1 
    s_steadyState_VOLT_SCALE = 1 (mV)
    s_q10Settings_fixedQ10 = 1 
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Navp type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Navp type=ionChannelHH), from gates; Component(id=m type=gateHHratesTau)
    ? multiply applied to all instances of fcond in: <gates> ([Component(id=m type=gateHHratesTau), Component(id=h type=gateHHratesTauInf), Component(id=s type=gateHHtauInf)]))
    fopen0 = m_fcond * h_fcond * s_fcond ? path based, prefix = 
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHratesTau), from timeCourse; Component(id=null type=Bezaire_Navp_taum)
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHratesTauInf), from timeCourse; Component(id=null type=Bezaire_Navp_tauh)
    ...
    s_timeCourse_T = temperature /  s_timeCourse_TEMP_SCALE ? evaluable
    s_timeCourse_alpha = exp((1e-3*12*( s_timeCourse_V +45)* s_timeCourse_F ) / ( s_timeCourse_R *( s_timeCourse_T +0.01))) ? evaluable
    s_timeCourse_beta = exp((1e-3*12*0.2*( s_timeCourse_V +45)* s_timeCourse_F ) / ( s_timeCourse_R *( s_timeCourse_T +0.01))) ? evaluable
    s_timeCourse_tmp = s_timeCourse_beta  / (3e-4*(1 +  s_timeCourse_alpha )) ? evaluable
    if (( s_timeCourse_tmp  /  s_rateScale ) < 10)  { 
        s_timeCourse_t = 10 *  s_rateScale  *  s_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        s_timeCourse_t = s_timeCourse_tmp  *  s_timeCourse_TIME_SCALE ? evaluable cdv
    }
    
    s_steadyState_V = v /  s_steadyState_VOLT_SCALE ? evaluable
    s_steadyState_alpha = 1 / (1 + exp(( s_steadyState_V +43)/2)) ? evaluable
    s_steadyState_x = s_steadyState_alpha  +  s_steadyState_no_inact *(1 -  s_steadyState_alpha ) ? evaluable
    s_q10Settings_q10 = s_q10Settings_fixedQ10 ? evaluable
    ? DerivedVariable is based on path: q10Settings[*]/q10, on: Component(id=s type=gateHHtauInf), from q10Settings; Component(id=null type=q10Fixed)
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=s type=gateHHtauInf), from steadyState; Component(id=null type=Bezaire_Navp_sss_ar2_1)
    s_inf = s_steadyState_x ? path based, prefix = s_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=s type=gateHHtauInf), from timeCourse; Component(id=null type=Bezaire_Navp_taus)
