TITLE Mod file for component: Component(id=Kdrfastngf type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Kdrfastngf type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Kdrfastngf type=ionChannelHH), from gates; Component(id=n type=gateHHrates)
...
PROCEDURE rates() {
    ...
    n_reverseRate_r = ((0.264) / (exp(( n_reverseRate_V +34)/40))) /  n_reverseRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=n type=gateHHrates), from reverseRate; Component(id=null type=Bezaire_Kdrfastngf_betaq)
