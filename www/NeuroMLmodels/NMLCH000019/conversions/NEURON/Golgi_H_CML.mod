TITLE Mod file for component: Component(id=Golgi_H_CML type=ionChannelHH)
...
PARAMETER {
    ...
    n_instances = 1 
    n_forwardRate_rate = 8.0000004E-4 (kHz)
    n_forwardRate_midpoint = -75 (mV)
    n_forwardRate_scale = -11.0011 (mV)
    n_reverseRate_rate = 8.0000004E-4 (kHz)
    n_reverseRate_midpoint = -75 (mV)
    n_reverseRate_scale = 11.0011 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Golgi_H_CML type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Golgi_H_CML type=ionChannelHH), from gates; Component(id=n type=gateHHrates)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=n type=gateHHrates), from forwardRate; Component(id=null type=HHExpRate)
