TITLE Mod file for component: Component(id=Na_STG type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -25.5 (mV)
    m_steadyState_scale = 5.29 (mV)
    h_instances = 1 
    h_timeCourse_TIME_SCALE = 1 (ms)
    h_timeCourse_VOLT_SCALE = 1 (mV)
    h_steadyState_rate = 1 
    h_steadyState_midpoint = -48.9 (mV)
    h_steadyState_scale = -5.18 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Na_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Na_STG type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (2.64 - 2.52/(1.0 + (exp (( m_timeCourse_V +120)/(-25))))) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=Na_STG_m_tau_tau)
    ...
    h_timeCourse_t = (1.34/(1.0 + (exp (( h_timeCourse_V  + 62.9)/(-10))))*(1.5 + 1.0/(1.0 + (exp (( h_timeCourse_V +34.9)/3.6))))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=Na_STG_h_tau_tau)
