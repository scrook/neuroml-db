TITLE Mod file for component: Component(id=Navcck type=ionChannelHH)
...
PARAMETER {
    ...
    m_forwardRate_midpoint = -44 (mV)
    m_forwardRate_scale = 5 (mV)
    m_reverseRate_rate = 1.5000001 (kHz)
    m_reverseRate_midpoint = -14.5 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Navcck type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Navcck type=ionChannelHH), from gates; Component(id=m type=gateHHrates)
...
PROCEDURE rates() {
    ...
    h_forwardRate_r = ( 0.35 / (exp(( h_forwardRate_V +65)/20))) /  h_forwardRate_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=h type=gateHHrates), from forwardRate; Component(id=null type=Bezaire_Navcck_alphah)
