TITLE Mod file for component: Component(id=SK type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=SK type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=SK type=ionChannelHH), from gates; Component(id=z type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=z type=gateHHtauInf), from steadyState; Component(id=null type=SK_z_inf_inf)
    z_inf = z_steadyState_x ? path based, prefix = z_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=z type=gateHHtauInf), from timeCourse; Component(id=null type=SK_z_tau_tau)
