TITLE Mod file for component: Component(id=Navapicalp type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Navapicalp type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Navapicalp type=ionChannelHH), from gates; Component(id=m type=gateHHratesTau)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=s type=gateHHtauInf), from steadyState; Component(id=null type=Bezaire_Navp_sss_ar2_0_8)
