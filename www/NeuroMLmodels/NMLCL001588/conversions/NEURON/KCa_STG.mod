TITLE Mod file for component: Component(id=KCa_STG type=ionChannelHH)
...
NEURON {
    ...
    RANGE m_timeCourse_CONC_SCALE           : parameter
    ...
    RANGE m_timeCourse_ca_conc              : derived variable
...
PARAMETER {
    ...
    m_timeCourse_CONC_SCALE = 1000000 (mM)
...
ASSIGNED {
    ...
    m_timeCourse_ca_conc                   : derived variable
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=KCa_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=KCa_STG type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_ca_conc = caConc /  m_timeCourse_CONC_SCALE ? evaluable
    m_timeCourse_t = (180.6 - 150.2/(1.0 + (exp (( m_timeCourse_V  + 46.0)/(-22.7))))) *  m_timeCourse_TIME_SCALE ? evaluable
    m_steadyState_V = v /  m_steadyState_VOLT_SCALE ? evaluable
    m_steadyState_ca_conc = caConc /  m_steadyState_CONC_SCALE ? evaluable
    m_steadyState_x = ( m_steadyState_ca_conc /( m_steadyState_ca_conc  + 3e-9)) * 1.0/(1.0 + (exp (( m_steadyState_V  + 28.3)/(-12.6)))) ? evaluable
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHtauInf), from steadyState; Component(id=null type=KCa_STG_m_inf_inf)
    m_inf = m_steadyState_x ? path based, prefix = m_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=KCa_STG_m_tau_tau)
