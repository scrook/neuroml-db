TITLE Mod file for component: Component(id=Golgi_Ca_LVA type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Golgi_Ca_LVA type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Golgi_Ca_LVA type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = (( 3 + 1 / ( (exp (( m_timeCourse_V +27)/ 10)) + (exp (( m_timeCourse_V +102)/(-15))) ) ) ) *  m_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_Ca_LVA_m_tau_tau)
    ...
    h_timeCourse_t = (( 85+ 1/ ( (exp (( h_timeCourse_V +48)/4)) + (exp (( h_timeCourse_V +407)/(-50))) ) ) ) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_Ca_LVA_h_tau_tau)
