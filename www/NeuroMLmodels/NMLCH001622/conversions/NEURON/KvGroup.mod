TITLE Mod file for component: Component(id=KvGroup type=ionChannelHH)
...
PARAMETER {
    ...
    n_instances = 1 
    n_forwardRate_rate = 0.12166001 (kHz)
    n_forwardRate_midpoint = 4.18371 (mV)
    n_forwardRate_scale = 6.42606 (mV)
    n_reverseRate_rate = 0.015857 (kHz)
    n_reverseRate_midpoint = 0 (mV)
    n_reverseRate_scale = -25.4834 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=KvGroup type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=KvGroup type=ionChannelHH), from gates; Component(id=n type=gateHHrates)
