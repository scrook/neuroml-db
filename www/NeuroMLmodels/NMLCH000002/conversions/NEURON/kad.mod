TITLE Mod file for component: Component(id=kad type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=kad type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=kad type=ionChannelHH), from gates; Component(id=n type=gateHHratesTauInf)
...
PROCEDURE rates() {
    ...
    n_forwardRate_r = ((exp ( (1e-3 * (-1.8 + (-1)/(1 + (exp (( n_forwardRate_V -(-40))/5)))) * ( n_forwardRate_V  - (-1)) * 9.648e4) / (8.315*(273.16 + ( n_forwardRate_celsius ) )) ))) /  n_forwardRate_TIME_SCALE ? evaluable
    ...
    n_reverseRate_r = ((exp ( (1e-3 * (-1.8 + (-1)/(1 + (exp (( n_reverseRate_V -(-40))/5)))) * 0.39 * ( n_reverseRate_V  - (-1)) * 9.648e4) / (8.315*(273.16 + ( n_reverseRate_celsius ) )) ))) /  n_reverseRate_TIME_SCALE ? evaluable
    ...
    } else if (n_timeCourse_BETA /(0.1 * (1 +  n_timeCourse_ALPHA ) *  n_rateScale )  < ( 0.2 ))  { 
        n_timeCourse_t = ( (0.2 *  n_rateScale ) ) *  n_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        n_timeCourse_t = (  n_timeCourse_BETA /(0.1 * (1 +  n_timeCourse_ALPHA )) ) *  n_timeCourse_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=n type=gateHHratesTauInf), from forwardRate; Component(id=null type=kad_n_alpha_rate)
    n_alpha = n_forwardRate_r ? path based, prefix = n_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=n type=gateHHratesTauInf), from reverseRate; Component(id=null type=kad_n_beta_rate)
    n_beta = n_reverseRate_r ? path based, prefix = n_
    
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=n type=gateHHratesTauInf), from steadyState; Component(id=null type=kad_n_inf_inf)
    n_inf = n_steadyState_x ? path based, prefix = n_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHratesTauInf), from timeCourse; Component(id=null type=kad_n_tau_tau)
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=l type=gateHHtauInf), from steadyState; Component(id=null type=kad_l_inf_inf)
    l_inf = l_steadyState_x ? path based, prefix = l_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=l type=gateHHtauInf), from timeCourse; Component(id=null type=kad_l_tau_tau)
