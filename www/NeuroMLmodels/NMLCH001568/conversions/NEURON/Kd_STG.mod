TITLE Mod file for component: Component(id=Kd_STG type=ionChannelHH)
...
PARAMETER {
    ...
    n_steadyState_midpoint = -12.3 (mV)
    n_steadyState_scale = 11.8 (mV)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=Kd_STG type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=Kd_STG type=ionChannelHH), from gates; Component(id=n type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    n_timeCourse_t = (14.4 - 12.8/(1.0 + (exp (( n_timeCourse_V  + 28.3)/(-19.2))))) *  n_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=n type=gateHHtauInf), from timeCourse; Component(id=null type=Kd_STG_n_tau_tau)
