TITLE Mod file for component: Component(id=K_P type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -14.3 (mV)
    m_steadyState_scale = 14.6 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=K_P type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=K_P type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    if (m_timeCourse_V   < ( -50 ))  { 
        m_timeCourse_t = ( (1.25 + 175.03 * (exp (( m_timeCourse_V ) * 0.026))) ) *  m_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
        m_timeCourse_t = ( (1.25 + 13 * (exp (( m_timeCourse_V ) * -0.026)))) *  m_timeCourse_TIME_SCALE ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=K_P_m_tau_tau)
    ...
    h_timeCourse_t = (360 + (1010 + 24*( h_timeCourse_V +55)) * (exp (-1 *(( h_timeCourse_V +75)/48)*(( h_timeCourse_V +75)/48)))) *  h_timeCourse_TIME_SCALE ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=K_P_h_tau_tau)
