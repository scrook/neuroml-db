...
<!--
    ...
    This file is used to convert v1.5 NetworkML files to a "neuroscientist friendly" HTML view
        ...
        <xsl:call-template name="tableRow">
            ...
            <xsl:with-param name="value">
                <xsl:for-each select="net:connection">
                        ...
                        &lt;p&gt;&lt;b&gt;<xsl:value-of select="@id"/>: From <xsl:if test="count(net:pre/@segment_id) &gt; 0">segment <xsl:value-of select="net:pre/@segment_id"/>
                            <xsl:if test="count(net:pre/@fraction_along) &gt; 0"> (fract along: <xsl:value-of select="net:pre/@fraction_along"/>)</xsl:if> on </xsl:if>
                            source cell <xsl:value-of select="net:pre/@cell_id"/>
                            to <xsl:if test="count(net:post/@segment_id) &gt; 0"> segment <xsl:value-of select="net:post/@segment_id"/>
                            <xsl:if test="count(net:post/@fraction_along) &gt; 0"> (fract along: <xsl:value-of select="net:post/@fraction_along"/>)</xsl:if> on </xsl:if>
                            target cell <xsl:value-of select="net:post/@cell_id"/>
...
<xsl:template match="net:target">
    ...
    <xsl:call-template name="tableRow">
        ...
        <xsl:with-param name="value">&lt;b&gt;<xsl:value-of select="@cell_group"/>&lt;/b&gt;</xsl:with-param>
    </xsl:call-template>     
    
    <xsl:if test="count(net:site) &gt; 0">

        <xsl:call-template name="tableRow">
            <xsl:with-param name="name">Cells receiving input:</xsl:with-param>
            <xsl:with-param name="value">
                <xsl:for-each select="net:site">
